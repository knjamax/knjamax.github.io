...
export function initSmokeyCursor(){
  ...
  const advect=`precision highp float; varying vec2 vUv; uniform sampler2D uTexture; uniform vec2 uPointer; uniform float uAspect,uTime,uActive,uClick;
    vec3 hue(float h){return .5+.5*cos(6.28318*(h+vec3(0.,.33,.67)));}
    void main(){vec2 p=vUv;vec2 d=p-uPointer;d.x*=uAspect;float dist=length(d);float wave=sin(p.y*7.0+uTime*.75+p.x*4.0)*.5+.5;vec2 tangent=vec2(-d.y,d.x)/(dist+.05);vec2 swirl=tangent*(.00135+uClick*.0022)*exp(-dist*dist*27.);vec2 push=uVelocity*.00072*exp(-dist*dist*52.);vec2 back=p-swirl-push;vec4 old=texture2D(uTexture,back);float fade=.957-uActive*.006-uClick*.003;float h=fract(.52+uTime*.018+p.x*.14+p.y*.11+wave*.12);vec3 colorA=hue(h);vec3 colorB=hue(h+.18);vec3 color=mix(colorA,colorB,smoothstep(.18,.82,wave));float splat=exp(-dist*dist*(175.-uClick*48.))*(uActive*.24+uClick*.58);float halo=exp(-dist*dist*72.)*(uActive*.045+uClick*.16);float ribbon=exp(-abs(d.y+sin(d.x*8.0+uTime*.7)*.025)*38.)*exp(-dist*dist*9.)*uActive*.025;vec3 dye=old.rgb*fade+color*(splat+halo+ribbon);gl_FragColor=vec4(clamp(dye,0.,1.),clamp(max(old.a*fade,splat+halo+ribbon),0.,1.));}`;
  const display='precision mediump float; varying vec2 vUv; uniform sampler2D uTexture; uniform float uOpacity; void main(){vec2 px=vec2(1.8/640.0,1.8/420.0); vec4 c=texture2D(uTexture,vUv)*.34; c+=texture2D(uTexture,vUv+vec2(px.x,0.))*0.165; c+=texture2D(uTexture,vUv-vec2(px.x,0.))*0.165; c+=texture2D(uTexture,vUv+vec2(0.,px.y))*0.165; c+=texture2D(uTexture,vUv-vec2(0.,px.y))*0.165; float m=max(max(c.r,c.g),c.b); float a=smoothstep(.006,.17,m)*uOpacity; gl_FragColor=vec4(c.rgb*a,a*.78);}';
  ...
  const draw=now=>{raf=requestAnimationFrame(draw);const dt=Math.min(40,now-last);last=now;frame++;const idle=now-lastMove>150;pointer.active=idle?Math.max(0,pointer.active-dt*.006):1;click=Math.max(0,click-dt*.0028);pointer.vx*=.84;pointer.vy*=.84;if(frame%2&&idle&&click<.02)return;
    ...
    gl.bindFramebuffer(gl.FRAMEBUFFER,null);gl.viewport(0,0,simW,simH);gl.useProgram(out);bind(out);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,dst.tex);gl.uniform1i(gl.getUniformLocation(out,'uTexture'),0);gl.uniform1f(gl.getUniformLocation(out,'uOpacity'),1.18);gl.drawArrays(gl.TRIANGLES,0,6);sw=1-sw;
