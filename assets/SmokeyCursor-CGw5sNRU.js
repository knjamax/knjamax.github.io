...
export function initSmokeyCursor(){
  ...
  const advect=`precision highp float; varying vec2 vUv; uniform sampler2D uTexture; uniform vec2 uPointer; uniform vec2 uVelocity; uniform float uAspect,uTime,uActive,uClick;
    vec3 hue(float h){return .5+.5*cos(6.28318*(h+vec3(0.,.33,.67)));}
    void main(){vec2 p=vUv;vec2 d=p-uPointer;d.x*=uAspect;float dist=length(d);vec2 swirl=vec2(-d.y,d.x)/(dist+.035)*(.0018+uClick*.0025)*exp(-dist*dist*34.);vec2 push=uVelocity*.00085*exp(-dist*dist*58.);vec2 back=p-swirl-push;vec4 old=texture2D(uTexture,back);float fade=.965-uActive*.008-uClick*.004;vec3 color=hue(fract(.54+uTime*.025+uPointer.x*.16+uPointer.y*.1));float splat=exp(-dist*dist*(210.-uClick*65.))*(uActive*.27+uClick*.72);float halo=exp(-dist*dist*95.)*(uActive*.035+uClick*.14);vec3 dye=old.rgb*fade+color*(splat+halo);gl_FragColor=vec4(clamp(dye,0.,1.),clamp(max(old.a*fade,splat+halo),0.,1.));}`;
  const display='precision mediump float; varying vec2 vUv; uniform sampler2D uTexture; uniform float uOpacity; void main(){vec2 px=vec2(1.35/640.0,1.35/420.0); vec4 c=texture2D(uTexture,vUv)*.42; c+=texture2D(uTexture,vUv+vec2(px.x,0.))*0.14; c+=texture2D(uTexture,vUv-vec2(px.x,0.))*0.14; c+=texture2D(uTexture,vUv+vec2(0.,px.y))*0.14; c+=texture2D(uTexture,vUv-vec2(0.,px.y))*0.14; float m=max(max(c.r,c.g),c.b); float a=smoothstep(.008,.2,m)*uOpacity; gl_FragColor=vec4(c.rgb*a,a*.86);}';
