...
export function initSmokeyCursor(){
  ...
  const display='precision mediump float; varying vec2 vUv; uniform sampler2D uTexture; uniform float uOpacity; void main(){vec4 c=texture2D(uTexture,vUv);float a=smoothstep(.012,.24,max(max(c.r,c.g),c.b))*uOpacity;gl_FragColor=vec4(c.rgb*a,a);}';
